...
public class Tasks {
	...
	public Tasks(int num) {
		...
		this.num = num;
		tasks = new int[num][num]; // Creates a marix of the tasks
	...
	private boolean dfs(int task, int[] result) {
		...
		// result
		if (inStack[task]) { // The task is in the recursion stack, in other words theres a cycle
			return false;
		}
		...
		inStack[task] = false; // Unmark task from recursion stack
	...
	public int[] order() {
		...
		// supposed to go
		int[] retArray = new int[num]; // Temporary array, will hold the tasks recieved from the method dfs
		...
		for (int i = 0; i < num; i++) {
			if (!visited[i]) {
				if (!dfs(i, retArray)) {
					return null; // Cycle detected, returns null
				}
			}
		}
		// retArray that was recieved in the DFS is topologically sorted
		return retArray;
	}
